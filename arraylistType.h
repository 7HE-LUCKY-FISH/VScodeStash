...
public:
    ...
    int sequentialSearch(const Type& object) const;
...
//sequentialSearch(): searches the arrayList for a specific item
//Arguments: object (const Type&)
//Returns: Location of object in the arrayList. -1 if object not found.
template <class Type>
int arrayListType<Type>::sequentialSearch(const Type& object) const {
    for(int index = 0; index < length; index++){
        Type objAtIndex;
        retrieveAt(index, objAtIndex);
        if(objAtIndex == object){
            return index;
        }
    }
    return -1;
